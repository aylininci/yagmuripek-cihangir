...
(() => {
  ...
  const music = {
    ...
    init() {
      ...
      this.el.volume = 0.5;
      this.el.load(); // yüklemeyi hemen tetikle
      ...
      // Herhangi bir ilk etkileşimde anında başlat (tarayıcı otomatik oynatma engeline karşı)
      const kick = () => this.start();
      ['pointerdown', 'touchstart', 'keydown', 'click'].forEach((ev) =>
        document.addEventListener(ev, kick, { passive: true })
      );

      // Otomatik denemeyi hem şimdi hem ses hazır olur olmaz yap
      this.start();
      this.el.addEventListener('canplay', () => this.start(), { once: true });
    },
    start() {
