...
(() => {
  ...
  /* ------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
  const music = {
    el: null, toggle: null, started: false, unlocked: false,
    init() {
      ...
      this.toggle.addEventListener('click', () => {
        this.unlocked = true;
        if (this.el.paused) { this.el.muted = false; this.el.play().catch(() => {}); this.toggle.classList.remove('is-muted'); }
        else { this.el.pause(); this.toggle.classList.add('is-muted'); }
      });

      // Strateji: sessiz (muted) olarak hemen çalmaya başla — bu mobilde
      // dahi izinlidir; ardından ilk kullanıcı temasında sesi aç.
      this.el.muted = true;
      this.tryPlay();
      this.el.addEventListener('canplay', () => this.tryPlay(), { once: true });
      ...
      // İlk gerçek etkileşimde sesi aç (her olay türünü dinle)
      ['pointerdown', 'touchend', 'touchstart', 'click', 'keydown'].forEach((ev) =>
    ...
    tryPlay() {
      ...
      if (p && p.then) {
        p.then(() => {
          this.started = true;
          if (!this.el.muted) this.toggle.classList.remove('is-muted');
        }).catch(() => { if (this.el.muted) { /* sessiz deneme; sorun değil */ } else this.toggle.classList.add('is-muted'); });
      }
    }
  };
  ...
  function playOpening() {
    ...
    if (reduce || !hasGSAP()) {
      ...
      setTimeout(() => { opening.style.display = 'none'; showEnvelope(); }, reduce ? 300 : 3000);
      return;
    }

    const tl = gsap.timeline();
    tl.to(card, { opacity: 1, scale: 1, duration: 1.1, ease: 'power2.out' })
      .to({}, { duration: 2.4 })                                   // toplam ~3.5sn ekranda
  ...
  /* ------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
  function boot() {
    music.init();     // müziği olabildiğince erken başlat
    initEnvelope();
    playOpening();
  }
